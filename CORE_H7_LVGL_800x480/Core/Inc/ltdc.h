...
typedef struct
{
	uint32_t pwidth;			//LCD面板的宽度,固定参数,不随�?�示方向改�?,如果为0,说�?�没有任何RGB屏接入
	uint32_t pheight;		//LCD面板的�?度,固定参数,不随�?�示方向改�?
	uint16_t hsw;			//水平同步宽度
	uint16_t vsw;			//垂直同步宽度
	uint16_t hbp;			//水平后廊
	uint16_t vbp;			//垂直后廊
...
extern uint32_t *ltdc_framebuf[2];					//LTDC LCD帧缓�?数组指针,必须指向对应大小的内�?区域
